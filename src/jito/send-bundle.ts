...
import type { BlockhashWithExpiryBlockHeight } from '../meteora/execute-txns-meteora';
...
const BLOCK_ENGINE_URL = "frankfurt.mainnet.block-engine.jito.wtf:443";
...
export const sendBundleNow = async (
    ...
    const tipAccounts = await client.getTipAccounts();
    if (!tipAccounts || !tipAccounts.length) {
        throw new Error("No Jito tip accounts found. Check your permission or key!");
    }
    const tipAccount = new PublicKey(tipAccounts[0]);
