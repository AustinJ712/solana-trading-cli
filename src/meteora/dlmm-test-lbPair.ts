...
async function extractPoolDetails(logMessages: string[], signature: string, tokenBalances: any) {
  ...
  for (const log of logMessages) {
    if (log.includes('Program log: Instruction: InitializeLbPair')) {
      // Pool address is typically the first account in the instruction
      const accountsLog = logMessages[logMessages.indexOf(log) - 1];
      if (accountsLog && accountsLog.includes('Program')) {
        const accounts = accountsLog.split(' ');
        poolAddress = accounts[accounts.length - 1];
        break;
      }
    }
  }
