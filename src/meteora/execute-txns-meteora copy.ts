...
import {
  ...
  export async function createAndSendMeteoraTx(
    ...
    const blockhashObj = await connection.getLatestBlockhash(METEORA_CONFIG.rpcCommitment);
    ...
    return sendTransactionWithRetries(connection, tx, signers, confirm);
  }
  ...
  export async function getOrCreateMeteoraAta(
    ...
    try {
      ...
      if (
        ...
        await createAndSendMeteoraTx(connection, [ix], [payer], true);
