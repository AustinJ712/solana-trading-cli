// /*
//  * DEPRECATED: This file is no longer used. See src/sniper.ts for the current implementation.
...
//   /**
//    * Build a proper swap transaction for Meteora
//    */
...
//     try {
//       // Determine if token is X or Y
//       const isTokenX = token.equals(this.accounts.token_mint_x);
//       const isTokenY = token.equals(this.accounts.token_mint_y);
      
//       if (!isTokenX && !isTokenY) {
//         throw new Error(`Token ${token.toBase58()} is neither tokenX nor tokenY of the pool`);
//       }
...
//         keys: [
//           { pubkey: this.accounts.lb_pair, isSigner: false, isWritable: true },
//           { pubkey: wallet, isSigner: true, isWritable: false },
//           { pubkey: isTokenX ? this.accounts.reserve_x : this.accounts.reserve_y, isSigner: false, isWritable: true },
//           { pubkey: isTokenX ? this.accounts.reserve_y : this.accounts.reserve_x, isSigner: false, isWritable: true },
//           { pubkey: this.accounts.oracle, isSigner: false, isWritable: true },
//           { pubkey: new PublicKey('TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'), isSigner: false, isWritable: false }, // Token program
//         ],
//         data: Buffer.concat([
//           sighash('swap'),
//           Buffer.from(new Uint8Array([
//             ...new BN(amount).toArray('le', 8),  // amount
//             ...new BN(0).toArray('le', 8),       // other_amount_threshold
//             ...new BN(0).toArray('le', 2),       // sqrt_price_limit
//             ...new Uint8Array([isTokenX ? 1 : 0])// zero_for_one (true if swapping token X for Y)
//           ]))
//         ])
//       };
...
//           programIdIndex: 0,
//           accountKeyIndexes: [1, 2, 3, 4, 5, 6],
//           data: swapIx.data,
//         }],
//         addressTableLookups: [],
//         staticAccountKeys: [
//           swapIx.programId,
//           ...swapIx.keys.map(k => k.pubkey)
//         ]
//       });

//       logger.info(`[MeteoraSniper] Successfully built swap transaction`);
//       return new VersionedTransaction(messageV0);
//     } catch (err) {
//       logger.error(`[MeteoraSniper] Failed to build swap transaction: ${err}`);
//       throw err;
//     }
//   }
